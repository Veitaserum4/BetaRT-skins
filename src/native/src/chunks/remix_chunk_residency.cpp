...
void RemixRenderer::computeFaceCoverage(ChunkMeshData& meshData) {
  ...
  const auto isSolidOccluder = [&](int index) -> bool {
      ...
      if (cell.materialClass != kOpaqueTerrainMaterialClass && cell.materialClass != kUnlitRedstoneOreTerrainMaterialClass && cell.materialClass != kLapisTerrainMaterialClass && cell.materialClass != kGlowstoneTerrainMaterialClass) return false;
